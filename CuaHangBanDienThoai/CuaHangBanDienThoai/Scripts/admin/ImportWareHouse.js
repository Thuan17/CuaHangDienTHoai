$(document).ready(function () {
    ...
    $(document).on('click', '.btnAddBill', function (event) {
        ...
        if (productid) {
            $.ajax({
                url: '/Admin/ImportWareHouse/EntryForm',
                data: {
                    productid: productid,
                    quantity: 1
                },
    ...
    $(document).on('click', '.btnDeleteItem', function (event) {
        ...
        if (productid) {
            $.ajax({
                url: '/Admin/ImportWareHouse/DeleteItemCart',
                data: { productid: productid },
                type: 'POST',
                success: function (res) {
                    if (res.Success && res.Code === 1) {
    ...
    $(document).on('click', '.btnquantity', function (event) {
        ...
        $.ajax({
            url: '/Admin/ImportWareHouse/UpdateQuantity',
            type: 'POST',
    ...
    $(document).on('input', '.inputQuantity', function () {
        ...
        if (newQuantity > 150) newQuantity = 150;
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.Success) {
                    createToast('success', 'fa-solid fa-check-circle', 'Thành công', 'Số lượng đã được cập nhật.');
                } else {
                    createToast('error', 'fa-solid fa-circle-exclamation', 'Thất bại', res.msg);
                }
            },
    ...
    $(document).on('change', '#supplierChose', function () {
       
        var supplierId = $(this).val();
        var selectedText = $("#supplierChose option:selected").text();
      
        if (supplierId) {
            $.ajax({
                url: '/Admin/ImportWareHouse/Partail_SupplierbyId',
                type: 'GET',
            ...
            alert("Lỗi nhận dữ liệu nhà cung cấp");
        }
    });
    ...
    $(document).on('click', '.btnImport', function (event) {
        ...
        if (supplierid) {
            ...
            $.ajax({
                url: '/Admin/ImportWarehouse/Import',
                data: { supplierid: supplierid },
                type: 'POST',
                success: function (res) {
                    if (res.Success) {
                        if (res.Code == 1 ) {
                            window.location.href = '/Admin/ImportWarehouse/DownloadInvoice?filePath=' + encodeURIComponent(res.FilePath);
...
function LoadListProduct() {
    $.ajax({
        url: '/Admin/ImportWareHouse/Partial_ListProductImport',
        type: 'GET',
        success: function (res) {
            LoadTotalPrice();
